...
const badgesName=[
  ...
  const handlePopupClosed = () => {
    ...
    <div>
    <h3 style={{color:"black",position: "fixed", top: "11rem", left: "7.6rem", right: 0}}>Badges:</h3>
    <ul className="menu" style ={{borderRadius:"20px",position: "fixed", top: "13.3rem", left: "2.3rem", right: 0, zIndex: 0 ,width:"15rem",marginLeft:"1rem" ,fontFamily: "Quicksand",fontWeight:"bold", color: "white",backgroundColor: "#FADA5E",border: "2px solid white"}}>
      {[1, 2, 3, 4, 5, 6].map(itemNumber => (
        <li
          key={itemNumber}
          className="menu-item"
          onClick={() => toggleDescription(itemNumber)}   style={{
            cursor: "pointer",
            padding: "0.5rem",
        ...
        >
          ...
          {activeItem === itemNumber && (
            <div className="description">
              <img src={badgess[itemNumber][0]} alt="badge" className="small-image"  style={{ '--small-image-border-color': badgess[itemNumber][1] ,marginLeft:"-1.6rem"}} ></img>
              <p style={{fontSize:"1.1rem",padding:0,margin:0}}>{badgess[itemNumber][2]}</p>
            </div>
          )}
        </li>
      ))}
    ...
    <Container >
      ...
      <Card  onClick={() => handleClick2(badgess[post.badges[0]][1],post.title,post.body,post.username,post.ID,post.timestamp,likedPosts.includes(post.ID) ? 1 : 0,post.comment_count,post.avatar)} key={index} className={`mt-4 ${index === posts.length - 1 ? "last-post" : ""} post-card`} style={{cursor: "pointer",marginLeft:"20rem",marginRight:"-20rem", backgroundColor: "#8CC4FF",borderRadius:"2rem",marginTop:"1rem" ,width:"61rem"}}>
        <Card.Body>
          <div className="row">
            <div className="col-sm-6">
              <div style={{ display: "flex", alignItems: "center" }}>
                <img src={avatars[post.avatar]} alt ='avatar' className="avatar-image" style = {{'--avatar-image-border-color': badgess[post.badges[0]][1],marginLeft:"1.2rem",width:"4rem",height:"4rem",borderWidth:"0.3rem",marginTop:"-1rem"}}></img>
                <div>
            ...
            <IconButton onClick={(e) => { e.stopPropagation();  handleLikeClick(post.ID, post.timestamp,username);}} style={{marginLeft: "1.3rem",marginTop:"-1.1rem",fontSize:"1.5rem",fontWeight:"bold",color: likedPosts.includes(post.ID) ?  "#DC143C" : "grey "}}><FavoriteIcon/>  </IconButton>
              ...
              <p className="card-text" style = {{marginLeft: "38.5rem", paddingTop:"0rem",marginTop:"0rem",fontSize:"1rem",fontWeight:"bold",color:"#FFFFFF"}}>{post.timestamp.slice(6,8)} - {post.timestamp.slice(4,6)} - {post.timestamp.slice(0,4)} </p>
            </div>
          </div>
        </Card.Body>
      </Card>
